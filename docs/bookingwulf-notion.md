...
- [x] **Kanalpreise pro OTA-Plattform** (Pro, 2026-08-27) — Preise getrennt pro Kanal (Airbnb, Booking.com, Expedia, Vrbo, HRS, Traumferienwohnungen, Agoda, Feratel) setzen, unabhängig vom eigenen Direktpreis. Läuft über Beds24s Offer-/PriceRules-Mechanismus (bis zu 16 Preis-Slots pro Zimmer), einmaliges Setup pro Apartment unter Beds24-Einstellungen ("Kanalpreise aktivieren"). Anlegen/Anzeigen/Bearbeiten in Kalender, Zimmerplan (inkl. reiner Tagespreis-Matrix "Preise") und eigener Verwaltungsseite `/admin/channel-prices` (Akkordeon nach Apartment/Kanal). Details: DOCUMENTATION.md §18.
...
- **2026-07-28 — Sentry-Wochenreport aufgearbeitet (DB-SSL + N+1):** Sentry meldete 7x SSL/TLS-Fehler bei `adminUser.findUnique` (Login-Check, läuft auf jeder Admin-Seite) und 4x „Connection terminated unexpectedly". Verdacht: `DATABASE_URL` in Vercel zeigte auf Railways internen Hostnamen (`railway.internal`, kein SSL) statt auf den öffentlichen Proxy (`proxy.rlwy.net`) — Variable ist in Vercel als „Sensitive" markiert und lässt sich nicht mehr auslesen, nur überschreiben; auf die öffentliche Railway-Proxy-URL neu gesetzt und redeployed. Zusätzlich 3 N+1-Query-Meldungen behoben: `src/lib/beds24.ts` lud `beds24Config` bei hotelweiten Sperrzeiten pro Apartment neu statt einmal fürs Hotel (durchs Sperrzeiten-Push-Feature vom 25.07. verursacht); die 4 Cron-Mails `checkin-email`/`checkout-reminder`/`pre-arrival-reminder`/`review-request` liefen je eine Request-Query pro Hotel in einer Schleife statt gebündelt — gleiches Muster wie der iCal-Sync-Fix im Juni, hier aber übersehen. **Lektion:** Bei neuen Cron-Jobs/Features mit Hotel-Iteration von Anfang an auf gebündelte Queries achten (`OR`-Array oder `hotelId: {in: [...]}`).

- **2026-07-25 — Beds24-Sperrzeiten-Push, ohne Feature-Flag ausgerollt:** Bewusst ohne Opt-in pro Hotel live geschaltet, obwohl technisch für alle 4 Beds24-verbundenen Häuser aktiv. Grund: Nur MSQ hat aktuell echte laufende OTA-Buchungen, ein Fehler beim Push (z.B. falsche Tages-Grenze bei inklusive/exklusive) betrifft also praktisch nur MSQ, deren Kollege die Korrektheit live verifiziert. Code-Review danach fand + fixte: alte `/admin/blocked-dates`-Seite umging den Sync komplett (jetzt angebunden), Reconciliation ignorierte andere Sperrzeit-Typen (jetzt alle berücksichtigt).
