  ...
  for (const r of blockedRanges) {
    ...
    while (cur < r.endDate) {
      if (cur.getFullYear() === year && cur.getMonth() === month) {
        ...
        dayBlockedMap.get(key)!.push({ id: r.id, aptName: r.apartment?.name ?? 'Alle Apartments', note: r.note ?? '', type: r.type, startDate: r.startDate.toISOString().slice(0,10), endDate: r.endDate.toISOString().slice(0,10), beds24SyncError: r.beds24SyncError });
      }
      cur.setDate(cur.getDate() + 1);
    }
  }
