    ...
    <div style={{ display: 'grid', gap: 12 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        {presets.length < 3 && !showNameInput && (
          <button type="button" onClick={() => setShowNameInput(true)} style={{ ...presetBtnStyle, background: 'var(--accent)', color: '#fff', border: 'none' }}>
            + Speichern
          </button>
        )}
      </div>
      ...
      {showNameInput && (
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          />
          <button type="button" onClick={savePreset} disabled={saving || !nameInput.trim()} style={{ ...presetBtnStyle, background: 'var(--accent)', color: '#fff', border: 'none' }}>
            {saving ? '…' : 'OK'}
          </button>
