...
const CONTENT = {
  ...
  features: {
    ...
    items: [
      { icon: '🚫',  title: 'Keine Provision',    desc: 'Sie behalten jeden Cent jeder Direktbuchung. Keine Transaktionsgebühren.' },
      { icon: '⚡',  title: 'Setup in 5 Minuten', desc: 'Eine Zeile Code — kein Entwickler nötig. Funktioniert mit WordPress, Framer, Wix & Co.' },
      { icon: '🎨',  title: 'Ihr Branding',        desc: 'Farben, Formen und Schrift passend zu Ihrer Website — komplett anpassbar.' },
      { icon: '🏠',  title: 'Multi-Apartment',     desc: 'Beliebig viele Einheiten mit individuellen Preisen, Bildern und Ausstattung.' },
      { icon: '🔄',  title: 'Channel Sync',        desc: 'Echtzeit-Sync mit Airbnb & Booking.com via Beds24 — keine Doppelbuchungen.' },
      { icon: '🇪🇺', title: 'DSGVO-konform',      desc: 'Alle Daten auf Servern in Deutschland — sicher und rechtskonform.' },
    ],
  },
  steps: {
...
export default function HomePage() {
  ...
  return (
    <>
      ...
      {/* ── Split-Hero ───────────────────────────────────────────── */}
      <section className="max-w-6xl mx-auto px-5 sm:px-8 py-16 sm:py-24 grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center" aria-labelledby="hero-heading">
          ...
          <ul className="flex flex-wrap gap-x-5 gap-y-1.5 list-none m-0 p-0" aria-label="Kernvorteile">
            {CONTENT.hero.trust.map((t) => (
              <li key={t} className="flex items-center gap-1.5 text-sm text-slate-500">
                <span className="text-green-500 font-bold text-base" aria-hidden>✓</span>
                {t}
              </li>
            ))}
          </ul>
      ...
      <section className="bg-slate-950 text-white py-20 px-5" aria-labelledby="pain-heading">
        <div className="max-w-6xl mx-auto text-center">
          ...
          <p className="text-slate-400 text-base leading-relaxed max-w-xl mx-auto mb-10 bw-animate bw-animate-delay-2">
            {CONTENT.pain.sub}
          </p>

          {/* Interaktiver Rechner */}
          <div className="bw-animate bw-animate-delay-2 max-w-xl mx-auto mb-10 bg-white/[0.05] rounded-2xl p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              ...
              <div key={item.label} className={`bw-animate bw-animate-delay-${i + 2} rounded-2xl p-6 transition-all duration-300 ${item.tone === 'positive' ? 'bg-green-500' : 'bg-white/[0.05]'}`}>
      ...
      {/* ── Features ─────────────────────────────────────────────── */}
      <section id="features" className="py-20 px-5 bg-white" aria-labelledby="features-heading">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="bw-animate mb-5 rounded-2xl border border-green-200 bg-linear-to-br from-green-50 to-emerald-50 p-7 flex flex-col sm:flex-row gap-5 items-start group hover:-translate-y-1 transition-all duration-300" style={{ boxShadow: 'var(--bw-shadow-card)' }}>
            <div className="text-4xl shrink-0 transition-transform duration-300 group-hover:scale-110" aria-hidden>{CONTENT.features.featured.icon}</div>
            <div>
          ...
          <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 list-none m-0 p-0" role="list">
            {CONTENT.features.items.map((f, i) => (
              <li key={f.title} className={`bw-card bw-animate bw-animate-delay-${Math.min(i + 1, 4)} p-6 group`}>
                <div className="text-2xl mb-3 inline-block transition-transform duration-300 group-hover:scale-110" aria-hidden>{f.icon}</div>
      ...
      {/* ── Pricing ──────────────────────────────────────────────── */}
      <section id="pricing" className="py-20 px-5 bg-white" aria-labelledby="pricing-heading">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 items-start">
            {plans.map(([key, plan], i) => (
              <div key={key} className={`bw-animate bw-animate-delay-${i + 1} relative rounded-2xl p-7 transition-all duration-300 ${key === 'pro' ? 'bg-slate-900 text-white shadow-2xl shadow-slate-200 hover:-translate-y-1.5' : 'bw-card hover:-translate-y-1'}`}>
                ...
                <Link href="/register" className={`bw-btn block text-center py-2.5 text-sm ${key === 'pro' ? 'bg-white text-slate-900 hover:bg-slate-100' : 'bg-transparent text-slate-900 border-2 border-slate-900 hover:bg-slate-900 hover:text-white'}`}>
                  Kostenlos testen
                </Link>
              </div>
            ))}
      ...
      {/* ── Founder ──────────────────────────────────────────────── */}
      <section className="py-20 px-5 bg-white" aria-labelledby="founder-heading">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-12">
          ...
          <div className="bw-animate bw-card max-w-2xl mx-auto p-8 flex flex-col sm:flex-row gap-8 items-center sm:items-start">
            {/* Foto-Placeholder — durch echtes Foto ersetzen */}
            <div className="w-24 h-24 rounded-full bg-slate-200 shrink-0 flex items-center justify-center text-3xl" aria-hidden>
              👤
            </div>
            <div>
      ...
      {/* ── Finaler CTA ──────────────────────────────────────────── */}
      <section className="py-24 px-5 bg-slate-950 text-white text-center" aria-labelledby="cta-heading">
        <div className="max-w-xl mx-auto bw-animate">
          ...
          <Link href="/register" className="bw-btn bg-white text-slate-900 hover:bg-slate-100 px-8 py-4 text-base font-bold rounded-xl">
  ...
  return (
    <div className={`bw-animate bw-animate-delay-${Math.min(index + 1, 4)} border-b border-slate-200`}>
      ...
      >
        ...
        <span className="text-slate-400 text-xl shrink-0 transition-transform duration-200" style={{ transform: open ? 'rotate(45deg)' : 'none' }} aria-hidden>+</span>
      </button>
