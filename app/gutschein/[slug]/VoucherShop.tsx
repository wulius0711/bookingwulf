  ...
  return (
    <html lang="de">
      <head>
        ...
        <title>Gutschein kaufen — {hotel.name}</title>
        <style>{`
          *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
          body { font-family: ui-sans-serif, system-ui, -apple-system, sans-serif; background: #f0f2f5; color: #111827; min-height: 100vh; }
          .wrap { max-width: 560px; margin: 0 auto; padding: 24px 16px 60px; }
          .card { background: #fff; border-radius: 16px; overflow: hidden; box-shadow: 0 2px 16px rgba(0,0,0,0.07); }
          .template-card { background: #fff; border-radius: 14px; border: 2px solid #e5e7eb; cursor: pointer; overflow: hidden; transition: border-color 0.15s, box-shadow 0.15s; }
          .template-card:hover { border-color: ${accent}88; box-shadow: 0 4px 16px rgba(0,0,0,0.08); }
          .template-card.selected { border-color: ${accent}; box-shadow: 0 4px 20px ${accent}33; }
          .btn { display: block; width: 100%; padding: 14px 20px; background: ${accent}; color: ${onAccent}; border: none; border-radius: 12px; font-size: 16px; font-weight: 700; cursor: pointer; font-family: inherit; transition: opacity 0.15s; }
